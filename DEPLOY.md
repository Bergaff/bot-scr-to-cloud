...
незагруженной сессии. Проверяет четыре вещи: заданы ли `TG_API_ID`/`TG_API_HASH` (и токен бота,
если в `RADAR_ARGS` есть `--notify bot`), настроен ли R2, пускают ли туда твои ключи
(ошибка `403` = опечатка в `R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY` либо у токена R2
недостаточно прав) и лежит ли в бакете `.session`.
...
```bash
python3 deploy/selftest_cloud.py      # 108 проверок: подпись R2, состояние, проход, эндпоинты, конфиг
...
npm test                              # то же самое одной командой (все 6 наборов, 495 проверок)
```
